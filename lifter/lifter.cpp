...
void asm_to_zydis_to_lift(lifterConcolic<>* lifter,
  ...
  while (lifter->getUnvisitedAddr(bbinfo, filter)) {
    ...
    std::cout << "[driver] lift start addr=0x" << std::hex
              << bbinfo.block_address << std::dec << " backup="
              << bbinfo.backupIndex << std::endl;

    lifter->liftBasicBlockFromAddress(bbinfo.block_address);

    std::cout << "[driver] lift end addr=0x" << std::hex << bbinfo.block_address
              << std::dec << " backup=" << bbinfo.backupIndex
              << " finished=" << lifter->finished << " run=" << lifter->run
              << std::endl;
  }
